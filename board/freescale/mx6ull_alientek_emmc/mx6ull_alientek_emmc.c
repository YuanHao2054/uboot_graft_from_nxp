...
// 删掉原本74LV595 的相关 GPIO，替换为SR8201F的复位引脚
...
// iox74lv_init() 和 iox74lv_set() 函数是原本74LV595的初始化函数，删除掉
...
static void setup_iomux_fec(int fec_id) //添加网络复位 IO 的初始化代码
{
	if (fec_id == 0)
	{
...
int board_init(void)
{
	...
	// 板子的初始化代码board_init()会调用74LV595的io初始化代码，删除掉
